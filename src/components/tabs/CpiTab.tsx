...
import { fetchCpi, ASEAN_COUNTRIES } from '../../services/apiService';
import LineChart from '../visualizations/LineChart';
import Table from '../visualizations/Table';
import LoadingSpinner from '../ui/LoadingSpinner';
import ErrorMessage from '../ui/ErrorMessage';
import { CpiDataItem, TimeRange } from '../../types/economicTypes';
...
const CpiTab: React.FC = () => {
  ...
  // Fetch CPI data for all selected countries in parallel
  const countryCodes = selectedCountries.map(c => c.value);
  const { data, isLoading, error } = useQuery(
    ['cpiMulti', countryCodes],
    async () => {
      const results = await Promise.all(
        countryCodes.map(code => fetchCpi(code))
      );
      return results;
    },
  ...
  if (!data || !data.length || !data[0] || !data[0].length) {
    return (
      <div className="slide-in">
        <div className="p-8 text-center text-slate-500">No CPI data available for the selected country/countries.</div>
      </div>
    );
  }
  ...
  const generateCsv = () => {
    ...
    const fileName = `cpi-comparison-${selectedCountries.map(c => c.value).join('-')}.csv`;
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
        <div>
          ...
          <p className="text-slate-600 mt-1">
            Compare Annual Consumer Price Index (CPI) for ASEAN Countries.
          </p>
        </div>
